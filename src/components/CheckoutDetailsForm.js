...
const CheckoutDetailsForm = () => {
    ...
    const handleCashPayment = async (e) => {
        ...
        try {
            // Add a new order to the user's orders collection
    ...
    useEffect(() => {
        ...
        const getClientSecret = debounce(async () => {
            const numericTotal = parseFloat(total);
            const adjustedTotal = numericTotal > 70 ? numericTotal : numericTotal + 10;
            const totalInCents = adjustedTotal * 100;

            try {
                const response = await axios({
                    method: 'post',
                    url: `/checkout/create?total=${totalInCents}`,
                });
        ...
        return () => {
            clearTimeout(getClientSecret); // Cleanup
        };
    }, [cart, total]);
    ...
    const handleAddress = async (e) => {
        ...
        try {
            // Add a new address to the user's storedAddresses collection
            const addressRef = doc(collection(db, 'users', user?.uid, 'storedAddresses'));
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(
            collection(db, 'users', user?.uid, 'storedAddresses'),
            (snapshot) => {
                setSavedAddresses(
